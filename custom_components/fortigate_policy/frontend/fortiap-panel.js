...
class FortiApPresencePanel extends HTMLElement {
  ...
  _styles() {
    return `<style>
      ...
      *{box-sizing:border-box}.shell{max-width:1500px;margin:0 auto;padding:24px 28px 110px}.top{position:relative;overflow:hidden;display:flex;align-items:center;justify-content:space-between;gap:24px;margin-bottom:18px;padding:22px 24px;background:var(--card-background-color);border:1px solid var(--divider-color);border-radius:18px;box-shadow:var(--ha-card-box-shadow,0 2px 8px rgba(0,0,0,.12))}.top:before{content:"";position:absolute;inset:0 auto 0 0;width:5px;background:var(--primary-color)}.brand{display:flex;align-items:center;gap:16px}.brand-mark{display:grid;place-items:center;width:52px;height:52px;border-radius:15px;background:color-mix(in srgb,var(--primary-color) 18%,var(--card-background-color));color:var(--primary-color)}.brand-mark ha-icon{--mdc-icon-size:30px}.eyebrow{color:var(--primary-color);font-weight:750;font-size:12px;text-transform:uppercase;letter-spacing:.1em}h1{font-size:30px;line-height:1.15;margin:4px 0 7px}h2{font-size:20px;margin:0 0 5px}h3{font-size:16px;margin:0 0 4px}.muted{color:var(--secondary-text-color)}
      .tabs{display:grid;grid-template-columns:repeat(4,minmax(150px,1fr));gap:8px;margin-bottom:20px;padding:7px;background:var(--card-background-color);border:1px solid var(--divider-color);border-radius:14px;box-shadow:var(--ha-card-box-shadow,0 1px 3px rgba(0,0,0,.08));overflow:auto}.tab{display:flex;align-items:center;justify-content:center;gap:9px;border:1px solid transparent;background:transparent;color:var(--secondary-text-color);padding:11px 14px;font:inherit;font-weight:650;cursor:pointer;border-radius:9px;white-space:nowrap}.tab ha-icon{--mdc-icon-size:20px}.tab:hover{background:var(--secondary-background-color);color:var(--primary-text-color)}.tab.active{color:var(--primary-color);background:color-mix(in srgb,var(--primary-color) 12%,var(--card-background-color));border-color:color-mix(in srgb,var(--primary-color) 35%,var(--divider-color))}
      .grid{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));gap:18px}.card{grid-column:span 12;background:var(--card-background-color);border:1px solid var(--divider-color);border-radius:16px;padding:20px;box-shadow:var(--ha-card-box-shadow,0 2px 7px rgba(0,0,0,.1))}.span-3{grid-column:span 3}.span-4{grid-column:span 4}.span-6{grid-column:span 6}.section-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:16px}.section-title{display:flex;align-items:center;gap:11px}.section-icon{display:grid;place-items:center;width:38px;height:38px;border-radius:11px;background:color-mix(in srgb,var(--primary-color) 13%,var(--card-background-color));color:var(--primary-color)}.section-icon ha-icon{--mdc-icon-size:22px}.metric-card{position:relative;overflow:hidden;display:flex;align-items:center;gap:15px;min-height:124px}.metric-card:after{content:"";position:absolute;inset:0 0 auto;height:3px;background:var(--metric-color)}.metric-card.blue{--metric-color:var(--panel-blue)}.metric-card.green{--metric-color:var(--panel-green)}.metric-card.purple{--metric-color:var(--panel-purple)}.metric-card.orange{--metric-color:var(--panel-orange)}.metric-icon{display:grid;place-items:center;flex:0 0 46px;width:46px;height:46px;border-radius:50%;color:var(--metric-color);background:color-mix(in srgb,var(--metric-color) 16%,var(--card-background-color))}.metric-icon ha-icon{--mdc-icon-size:25px}.metric-label{font-size:13px;font-weight:650;color:var(--secondary-text-color)}.metric{font-size:30px;font-weight:750;line-height:1.05;margin:4px 0}.metric-detail{font-size:12px;color:var(--secondary-text-color);line-height:1.35}.status{display:inline-flex;align-items:center;gap:5px;border-radius:999px;padding:5px 10px;font-size:12px;font-weight:700;background:var(--secondary-background-color)}.status.home,.status.enable,.status.ok{color:#20a05a;background:#20a05a1f}.status.not_home,.status.disable{color:#d97706;background:#f59e0b22}.status.unavailable,.status.pending{color:var(--secondary-text-color)}
      ...
      .savebar{position:fixed;z-index:5;left:0;right:0;bottom:0;background:color-mix(in srgb,var(--card-background-color) 94%,transparent);backdrop-filter:blur(12px);border-top:1px solid var(--divider-color);padding:14px 28px}.save-inner{max-width:1444px;margin:auto;display:flex;justify-content:space-between;align-items:center;gap:16px}.save-note{display:flex;align-items:center;gap:10px}.save-note ha-icon{color:var(--primary-color);--mdc-icon-size:24px}.empty{text-align:center;padding:42px;color:var(--secondary-text-color)}
  ...
  _overview() {
    ...
    return `<div class="grid">
      ${this._metric("Tracked devices", d.trackers.length, "Selected MAC addresses only", "mdi:cellphone-link", "blue")}
      ${this._metric("People", d.users.length, "Multi-device presence groups", "mdi:account-group", "green")}
      ${this._metric("Policy rules", d.rules.length, "Unknown state blocks writes", "mdi:source-branch", "purple")}
      ${this._metric("Firewall policies", d.policies.length, "Status-only verified control", "mdi:shield-check", "orange")}
      <section class="card span-6"><div class="section-head"><div class="section-title"><span class="section-icon"><ha-icon icon="mdi:router-wireless"></ha-icon></span><div><h2>Connection</h2><div class="muted">FortiGate management path</div></div></div><span class="status ${health.wifi_available ? "ok" : "unavailable"}"><ha-icon icon="${health.wifi_available ? "mdi:check-circle" : "mdi:alert-circle"}"></ha-icon>${health.wifi_available ? "Wi-Fi API available" : "Wi-Fi API unavailable"}</span></div>
      ...
      <section class="card span-6"><div class="section-title"><span class="section-icon"><ha-icon icon="mdi:shield-lock-outline"></ha-icon></span><div><h2>Safety behavior</h2><div class="muted">Conservative presence and policy control</div></div></div><div class="stack muted" style="margin-top:16px"><div>Arrival is immediate after a valid FortiAP association poll.</div><div>Departure waits ${escapeHtml(d.settings.wifi_away_grace_period)} seconds.</div><div>FortiGate failure produces unavailable, never away.</div><div>Policy writes change status only and are read back for verification.</div></div></section>
      <section class="card"><div class="section-head"><div class="section-title"><span class="section-icon"><ha-icon icon="mdi:account-multiple-check"></ha-icon></span><div><h2>People at a glance</h2><div class="muted">All assignments without opening an editor</div></div></div></div>${this._peopleSummary()}</section>
    </div>`;
  }

  _metric(title, value, detail, icon, tone) {
    return `<section class="card span-3 metric-card ${escapeHtml(tone)}"><span class="metric-icon"><ha-icon icon="${escapeHtml(icon)}"></ha-icon></span><div><div class="metric-label">${escapeHtml(title)}</div><div class="metric">${escapeHtml(value)}</div><div class="metric-detail">${escapeHtml(detail)}</div></div></section>`;
  }
  ...
  _render() {
    ...
    const tabs = [["overview","Overview","mdi:view-dashboard-outline"],["devices","People & devices","mdi:account-multiple-outline"],["policies","Policies & rules","mdi:shield-account-outline"],["settings","Settings","mdi:cog-outline"]];
    this.shadowRoot.innerHTML = `${this._styles()}<div class="shell"><header class="top"><div class="brand"><span class="brand-mark"><ha-icon icon="mdi:access-point-network"></ha-icon></span><div><div class="eyebrow">FortiAP Presence Tracker ${escapeHtml(this._draft.version)}</div><h1>Wireless presence and policy control</h1><div class="muted">${escapeHtml(this._draft.title)} · ${escapeHtml(this._draft.connection.vdom)} VDOM</div></div></div><div class="row">${this._entries.length > 1 ? `<select data-action="entry-select">${this._entries.map((entry) => `<option value="${escapeHtml(entry.entry_id)}" ${entry.entry_id === this._draft.entry_id ? "selected" : ""}>${escapeHtml(entry.title)}</option>`).join("")}</select>` : ""}<a class="btn" href="/home" aria-label="Back to Home Assistant overview"><ha-icon icon="mdi:home-assistant"></ha-icon>Home Assistant</a><button class="btn" data-action="reload"><ha-icon icon="mdi:refresh"></ha-icon>Refresh</button></div></header>${this._error ? `<div class="notice error">${escapeHtml(this._error)}</div>` : ""}${this._notice ? `<div class="notice">${escapeHtml(this._notice)}</div>` : ""}<nav class="tabs" aria-label="FortiAP Presence sections">${tabs.map(([key,label,icon]) => `<button class="tab ${this._tab === key ? "active" : ""}" data-action="tab" data-tab="${key}" ${this._tab === key ? 'aria-current="page"' : ""}><ha-icon icon="${icon}"></ha-icon>${label}</button>`).join("")}</nav>${views[this._tab]}</div><div class="savebar"><div class="save-inner"><div class="save-note"><ha-icon icon="mdi:content-save-check-outline"></ha-icon><div><strong>Review changes before saving</strong><div class="muted">Policy IDs are verified and the integration reloads atomically.</div></div></div><button class="btn primary" data-action="save" ${this._saving ? "disabled" : ""}><ha-icon icon="mdi:content-save-outline"></ha-icon>${this._saving ? "Saving…" : "Save changes"}</button></div></div>`;
