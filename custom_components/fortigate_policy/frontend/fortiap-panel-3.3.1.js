...
class FortiApPresencePanel extends HTMLElement {
  ...
  _styles() {
    return `<style>
      :host{display:block;min-height:100vh;background:var(--primary-background-color,#101010);color:var(--primary-text-color,#f5f5f5);font-family:var(--paper-font-body1_-_font-family,system-ui,sans-serif);--panel-blue:#03a9f4;--panel-green:#2eaf68;--panel-purple:#9c6ade;--panel-orange:#f59e0b;--panel-surface:color-mix(in srgb,var(--primary-text-color,#f5f5f5) 7%,var(--primary-background-color,#101010));--panel-surface-strong:color-mix(in srgb,var(--primary-text-color,#f5f5f5) 11%,var(--primary-background-color,#101010));--panel-border:color-mix(in srgb,var(--primary-text-color,#f5f5f5) 14%,transparent)}
      *{box-sizing:border-box}.mdi-icon{display:block;width:1em;height:1em;flex:0 0 auto;object-fit:contain}.shell{max-width:1500px;margin:0 auto;padding:24px 28px 110px}.top{position:relative;overflow:hidden;display:flex;align-items:center;justify-content:space-between;gap:24px;margin-bottom:18px;padding:22px 24px;background:var(--panel-surface);border:1px solid var(--panel-border);border-radius:18px;box-shadow:var(--ha-card-box-shadow,0 2px 8px rgba(0,0,0,.12))}.top:before{content:"";position:absolute;inset:0 auto 0 0;width:5px;background:var(--primary-color)}.brand{display:flex;align-items:center;gap:16px}.brand-mark{display:grid;place-items:center;width:52px;height:52px;border-radius:15px;background:color-mix(in srgb,var(--primary-color) 18%,var(--panel-surface));color:var(--primary-color)}.brand-mark .mdi-icon{width:30px;height:30px}.eyebrow{color:var(--primary-color);font-weight:750;font-size:12px;text-transform:uppercase;letter-spacing:.1em}h1{font-size:30px;line-height:1.15;margin:4px 0 7px}h2{font-size:20px;margin:0 0 5px}h3{font-size:16px;margin:0 0 4px}.muted{color:var(--secondary-text-color)}
      .tabs{display:grid;grid-template-columns:repeat(4,minmax(150px,1fr));gap:8px;margin-bottom:20px;padding:7px;background:var(--panel-surface);border:1px solid var(--panel-border);border-radius:14px;box-shadow:var(--ha-card-box-shadow,0 1px 3px rgba(0,0,0,.08));overflow:auto}.tab{display:flex;align-items:center;justify-content:center;gap:9px;border:1px solid transparent;background:transparent;color:var(--secondary-text-color);padding:11px 14px;font:inherit;font-weight:650;cursor:pointer;border-radius:9px;white-space:nowrap}.tab .mdi-icon{width:20px;height:20px}.tab:hover{background:var(--panel-surface-strong);color:var(--primary-text-color)}.tab.active{color:var(--primary-color);background:color-mix(in srgb,var(--primary-color) 14%,var(--panel-surface));border-color:color-mix(in srgb,var(--primary-color) 40%,var(--panel-border))}
  ...
  _policies() {
    return `<div class="grid"><section class="card"><div class="section-head"><div><h2>Firewall policies</h2><div class="muted">Optional. IDs are verified against FortiGate before saving.</div></div><div class="row add-row"><input id="new-policy-id" inputmode="numeric" placeholder="Policy ID"><button class="btn" data-action="add-policy">Add policy</button></div></div>${this._draft.policies.length ? `<table class="mobile-table"><thead><tr><th>Status</th><th>ID</th><th>Verified policy name</th><th></th></tr></thead><tbody>${this._draft.policies.map((policy) => `<tr><td data-label="Status"><span class="status ${escapeHtml(policy.state)}">${escapeHtml(policy.state)}</span></td><td data-label="Policy ID" class="mono">${escapeHtml(policy.id)}</td><td data-label="Policy name">${escapeHtml(policy.name)}</td><td class="right mobile-actions"><button class="icon-btn" data-action="remove-policy" data-id="${escapeHtml(policy.id)}">Remove</button></td></tr>`).join("")}</tbody></table>` : `<div class="empty">Tracker-only mode. No firewall policy control is configured.</div>`}</section>
