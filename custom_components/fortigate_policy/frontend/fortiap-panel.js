...
class FortiApPresencePanel extends HTMLElement {
  ...
  _styles() {
    return `<style>
      :host{display:block;min-height:100vh;background:var(--primary-background-color);color:var(--primary-text-color);font-family:var(--paper-font-body1_-_font-family,system-ui,sans-serif)}
      *{box-sizing:border-box}.shell{max-width:1500px;margin:0 auto;padding:24px 28px 110px}.top{display:flex;align-items:flex-start;justify-content:space-between;gap:24px;margin-bottom:22px}.eyebrow{color:var(--primary-color);font-weight:700;font-size:13px;text-transform:uppercase;letter-spacing:.08em}h1{font-size:32px;line-height:1.15;margin:5px 0 8px}h2{font-size:21px;margin:0 0 6px}h3{font-size:16px;margin:0 0 4px}.muted{color:var(--secondary-text-color)}
      .tabs{display:flex;gap:8px;overflow:auto;border-bottom:1px solid var(--divider-color);margin-bottom:24px}.tab{border:0;background:transparent;color:var(--secondary-text-color);padding:13px 16px;font:inherit;font-weight:650;cursor:pointer;border-bottom:3px solid transparent;white-space:nowrap}.tab.active{color:var(--primary-color);border-color:var(--primary-color)}
      .grid{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));gap:18px}.card{grid-column:span 12;background:var(--card-background-color);border:1px solid var(--divider-color);border-radius:14px;padding:20px;box-shadow:var(--ha-card-box-shadow,0 1px 2px rgba(0,0,0,.08))}.span-3{grid-column:span 3}.span-4{grid-column:span 4}.span-6{grid-column:span 6}.section-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:16px}.metric{font-size:30px;font-weight:750;margin-top:8px}.status{display:inline-flex;align-items:center;border-radius:999px;padding:4px 9px;font-size:12px;font-weight:700;background:var(--secondary-background-color)}.status.home,.status.enable,.status.ok{color:#138a4b;background:#138a4b1a}.status.not_home,.status.disable{color:#b05200;background:#ff98001a}.status.unavailable,.status.pending{color:var(--secondary-text-color)}
      ...
      button{font:inherit}.btn{display:inline-flex;align-items:center;justify-content:center;border:1px solid var(--divider-color);background:var(--card-background-color);color:var(--primary-text-color);border-radius:9px;padding:9px 13px;font-weight:650;cursor:pointer;text-decoration:none}.btn.primary{background:var(--primary-color);border-color:var(--primary-color);color:var(--text-primary-color,#fff)}.btn.danger{color:var(--error-color)}.btn:disabled{opacity:.55;cursor:wait}.icon-btn{border:0;background:transparent;color:var(--error-color);cursor:pointer;padding:8px}
  ...
  _overview() {
    ...
    return `<div class="grid">
      ${this._metric("Tracked devices", d.trackers.length, "Selected MAC addresses only")}
      ${this._metric("People", d.users.length, "Multi-device presence groups")}
      ${this._metric("Policy rules", d.rules.length, "Unknown state blocks writes")}
      ${this._metric("Firewall policies", d.policies.length, "Status-only verified control")}
      <section class="card span-6"><div class="section-head"><div><h2>Connection</h2><div class="muted">FortiGate management path</div></div><span class="status ${health.wifi_available ? "ok" : "unavailable"}">${health.wifi_available ? "Wi-Fi API available" : "Wi-Fi API unavailable"}</span></div>
      ...
      <section class="card span-6"><h2>Safety behavior</h2><div class="stack muted"><div>Arrival is immediate after a valid FortiAP association poll.</div><div>Departure waits ${escapeHtml(d.settings.wifi_away_grace_period)} seconds.</div><div>FortiGate failure produces unavailable, never away.</div><div>Policy writes change status only and are read back for verification.</div></div></section>
      <section class="card"><div class="section-head"><div><h2>People at a glance</h2><div class="muted">All assignments without opening an editor</div></div></div>${this._peopleSummary()}</section>
    </div>`;
  }

  _metric(title, value, detail) {
    return `<section class="card span-3"><div class="muted">${escapeHtml(title)}</div><div class="metric">${escapeHtml(value)}</div><div class="muted">${escapeHtml(detail)}</div></section>`;
  }
